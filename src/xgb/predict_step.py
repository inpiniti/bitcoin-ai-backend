...
async def handler(event, context):
    ...
    if dataset_id:
        try:
            # datasetId가 있으면 Supabase에서 로드 (predict용 함수 정의 필요)
            # 여기서는 편의상 내부 함수로 포함하거나 train_step 코드를 가져옴
            # 하지만 import가 어려우므로 직접 구현
    ...
    # 1개 데이터 또는 다수 데이터 처리
    # input_data가 1D array라면 (feature 1줄) -> (1, N)으로 reshape
    # 2D array라면 (M, N) 그대로 사용
