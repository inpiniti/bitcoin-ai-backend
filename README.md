...
스탭1 : 트레이딩뷰에서 미국 주식 조회 -> 
스탭2 : 데이터와 시총을 분리 ->

데이터는
 총마진, 영업마진, 세전마진, 넷마진, fcf마진, roa, roe, 투하자본수익률, r&d 비율, 판관비율,
 수입, 매출성장률, eps희석, eps 희석성장, 자산총액, 유동자산, 현금, 부채총액, 총부채, 순부채, 토탈 에쿼티, 유동비율, 당좌비율, 부채 / 자본, 현금 / 부채, 영업현금흐름, 투자현금흐름, 재무현금흐름, fcf, capex,
 위와 같은 데이터를 주로 사용하면 좋겠다.

그외 섹터같은 데이터도 포함하는데, 이런건 원-핫 인코딩으로 

스탭3 :  전처리 -> 
 
스탭4 : 텐서플로우로 시총을 알아내는 모델 생성 ->
스탭5 : 해당 모델로 특정 주식의 시총을 알아냄 